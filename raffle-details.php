...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'apply_discount') {
        $request = new WP_REST_Request('POST', '/rk/local-cart-discount');
        $response = rk_apply_recovery_discount($request);
        echo wp_json_encode(is_wp_error($response) ? ['success' => false, 'message' => $response->get_error_message()] : ($response instanceof WP_REST_Response ? $response->get_data() : $response));
        exit;
    }

    // Proxy: Transfer Funds (Winnings -> Wallet)
    if ($action === 'transfer') {
        $request = new WP_REST_Request('POST', '/rk/local-transfer');
        $request->set_body_params(['amount' => $_POST['amount'] ?? 0]);
        $response = rk_handle_transfer($request);
        echo wp_json_encode(is_wp_error($response) ? ['success' => false, 'message' => $response->get_error_message()] : ($response instanceof WP_REST_Response ? $response->get_data() : $response));
        exit;
    }
}
...
if ($raffle_id > 0) {
    // Fetch raffle data directly from the local WordPress bootstrap.
    $raffle_post = get_post($raffle_id);
    if ($raffle_post && $raffle_post->post_type === 'raffle') {
        $raffle_data = [
            'id' => $raffle_post->ID,
...
<script>
    ...
    async function fetchRaffleDetails() {
        try {
            const res = await fetch(`ajax-router.php?action=get_raffle&id=${encodeURIComponent(ssrRaffleId)}`);
