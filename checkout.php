    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                const res = await fetch(API_CONFIG.SETTINGS);
                if(res.ok) {
            ...
            try {
                const baseUrl = (typeof WORDPRESS_URL !== 'undefined') ? WORDPRESS_URL : '';
                const res = await fetch(API_CONFIG.BALANCE, {
        ...
        async function fetchUpsellIds() {
             try {
                const baseUrl = (typeof WORDPRESS_URL !== 'undefined') ? WORDPRESS_URL : '';
                const res = await fetch(`ajax-router.php?action=get_raffles&per_page=20&status=publish`);
                if(!res.ok) return;
        ...
        async function syncCheckoutToBackend(token) {
            ...
            try {
                await fetch(API_CONFIG.CART_SYNC, {
                    method: 'POST',
                    headers: {
        ...
        async function executeTransfer() {
            ...
            try {
                const res = await fetch(API_CONFIG.TRANSFER, {
                    method: 'POST',
        ...
        async function processPayment() {
            ...
            const token = localStorage.getItem('token');
            const baseUrl = (typeof WORDPRESS_URL !== 'undefined') ? WORDPRESS_URL : '';
            const ENDPOINT = API_CONFIG.PAYMENT;
