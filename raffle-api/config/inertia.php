...
return [
    ...
    */
    ...
    |
    | NOTE: the package default here is `resources/js/pages` (lowercase) —
    | this app's actual directory is `resources/js/Pages` (capital P, the
    | convention every Inertia starter kit and this app's own pages use).
    | On a case-sensitive filesystem (Linux, every real deploy target) the
    | default silently never finds a single page, which only surfaces when
    | something calls `assertInertia(...)->component(...)` in a test. Found
    | and fixed while adding item 25's route tests.
    |
    */
